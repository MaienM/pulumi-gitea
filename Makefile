...
SHELL            := /bin/bash
PROJECT          := github.com/MaienM/pulumi-gitea
NODE_MODULE_NAME := @maienm/pulumi-gitea
TF_NAME          := gitea
...
TFGEN            := pulumi-tfgen-gitea
PROVIDER         := pulumi-resource-gitea
...
upstream/.git:
	...
	git -c advice.detachedHead=false clone --depth 1 --branch 'v$(PROVIDER_VERSION)' 'https://github.com/go-gitea/terraform-provider-gitea' upstream
