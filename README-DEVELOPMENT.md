...
`terraform-provider-gitea`.  Substitute
...
1. Create the schema by running the following command:
    ...
    ```text
    warning: resource gitea_something not found in provider map; skipping
    warning: resource gitea_something_else not found in provider map; skipping
    warning: data source gitea_something not found in provider map; skipping
    warning: data source gitea_something_else not found in provider map; skipping
    ```
...
1. **Add resource mappings:** For each resource in the provider, add an entry in
   the `Resources` property of the `tfbridge.ProviderInfo`, e.g.:
    ...
    "gitea_something":      {Tok: tfbridge.MakeResource(mainPkg, mainMod, "Something")},
    "gitea_something_else": {Tok: tfbridge.MakeResource(mainPkg, mainMod, "SomethingElse")},
    ```
...
1. **Add data source mappings:** For each data source in the provider, add an entry in the `DataSources` property of the `tfbridge.ProviderInfo`, e.g.:
    ...
    "gitea_something":      {Tok: tfbridge.MakeDataSource(mainPkg, mainMod, "getSomething")},
    "gitea_something_else": {Tok: tfbridge.MakeDataSource(mainPkg, mainMod, "getSomethingElse")},
    ```
...
1. **Add provider configuration overrides (not typically needed):** Pulumi's
   ...
   `gitea_API_KEY`.  Examples of common
   uses cases:
